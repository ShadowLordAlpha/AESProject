...
public final class AES {
	...
	public static final byte[] decrypt(byte[] key, byte[] data) {
		/*
		 * We do not allow for the key or the data to be null
		 */
		if (key == null || data == null) {
	...
	public static final ByteBuffer keyExpansion(byte[] key, int size, int nK) {
		...
		for (int i = nK; i < size; i++) {
			...
			if ((i % nK) == 0) {
				...
				// This is here so we can reuse another function and the work
				// done for rotWord.
				...
				temp = subWord ^ rcon;
				// System.out.printf("\t%8x\t%8x\t%8x\t%8x", rotWord, subWord,
				// rcon, temp); // DEBUG
			} else if ((nK > 6) && ((i % nK) == 4)) {
				...
				int subWord = work.getInt(0);
				temp = subWord;
				// System.out.printf("\t%8x\t%8x\t%8x\t%8x", 0, subWord, 0,
				// temp); // DEBUG
			} else {
				// System.out.printf("\t%8x\t%8x\t%8x\t%8x", 0, 0, 0, 0); //
				// DEBUG (this makes it look nice)
			}
	...
	private static final class Cypher {
		 ...
		 * affine transformation already applied and placed into a 1d array for
		 * easy access.
		 * 
		 * @param state
		 * @return
		 ...
		 * @return
		 */
		private static final byte GMul(byte a, byte b) { // Galois Field (256)
															// Multiplication of
															// two Bytes
			byte p = 0;
			byte counter;
	...
	private static final class InvCypher {
		...
		private static final byte[] dec(byte[] key, byte[] state) {
			// number of words in the key
			int nK = (key.length * 8) / 32;
			// number of words per block
			int nB = 4; // this is always 4 as the block size is constant in AES
			...
			//System.out.println("[ 0 ] "+AES.bytesToHex(state));
			state = addRoundKey(state, w, nR);
			...
			state = invSubBytes(state);
			state = addRoundKey(state, w, 0);
			//System.out.println("Output " + AES.bytesToHex(state));
			return state;
		}

		/**
		 * Implementation of section 5.3.2 using a pre-calculated s-box with
		 * affine transformation already applied and placed into a 1d array for
		 * easy access.
		 * 
		 * @param state
		 * @return
		 */
		private static final byte[] invSubBytes(byte[] state) {
		 ...
		 * @return
		 */
		private static final byte[] invShiftRows(byte[] state) {
			...
			for (int i = 0; i < 4; i++) {
				...
				for (int j = 0; j < i; j++) {
					// NOTE: This could also be hardcoded to properly shift all
					// values instead of a loop
					byte temp = state[i + 12]; // get the last element in
											// a row
					// Shift the matrix row
		 ...
		 * implementation of section 5.1.3
		 */
		private static final byte[] invMixColumns(byte[] state) {
			...
			for (int i = 0; i < 4; i++) {
				...
				state[0 + (i * 4)] = (byte) (GMul((byte) 0x0E, s0) ^ GMul((byte) 0x0B, s1) ^ GMul((byte) 0x0D, s2) ^ GMul((byte) 0x09, s3));
				state[1 + (i * 4)] = (byte) (GMul((byte) 0x09, s0) ^ GMul((byte) 0x0E, s1) ^ GMul((byte) 0x0B, s2) ^ GMul((byte) 0x0D, s3));
				state[2 + (i * 4)] = (byte) (GMul((byte) 0x0D, s0) ^ GMul((byte) 0x09, s1) ^ GMul((byte) 0x0E, s2) ^ GMul((byte) 0x0B, s3));
				state[3 + (i * 4)] = (byte) (GMul((byte) 0x0B, s0) ^ GMul((byte) 0x0D, s1) ^ GMul((byte) 0x09, s2) ^ GMul((byte) 0x0E, s3));
			}
